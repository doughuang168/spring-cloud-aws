		...
		upload: {
			...
			init: function(){
				...
				this.target.setConfig({
					...
					// --------- e
					onClickUploadedItem: function(){ // When you click the Upload List.
						window.open(this.uploadedPath.dec() + this.saveName.dec(), "_blank", "width=500,height=500");
					},

					uploadMaxFileSize:(1000*1024*1024), // Size limited to an individual in the uploaded file size (the client and not a value set by the server.)
					uploadMaxFileCount:10, // 0 uploaded files count limit is unlimited
					uploadUrl:"/api/aws/s3/upload",
					uploadPars:{},
					deleteUrl:"/api/aws/s3/upload",
					deletePars:{},

					buttonTxt:'<i class="axi axi-cloud-upload"></i>&nbsp;&nbsp; Upload file',

					fileKeys:{ // json key definitions that are returned by the server (id is not available reserved)
						//id:"id",
						name:"name",
						type:"type",
						saveName:"saveName",
						...
						thumbPath:"thumbUrl" // One example is the different treatment of names to check that the key values on the server can be set differently.
					},
					...
					onComplete: function(){
						//trace(this);
						toast.push(target.uploadedList.length + "This suggests a file has been uploaded.");

						target.setUploadedList([]);
						$("#uploadQueueBox").empty();

						fnObj.grid.query();
						// $("#uploadCancelBtn").get(0).disabled = true; // Transfer Stop button control
					},
					onStart: function(){
						//trace(this);
						//trace("onStart");
						// $("#uploadCancelBtn").get(0).disabled = false; // Transfer Stop button control
					},
					onDelete: function(){
						trace(this);
						//trace("onDelete");
					...
					onError: function(errorType, extData){
						if(errorType == "html5Support"){
							...
							//trace(extData);
							alert("You can not upload a file size exceeds the file. Exclude from your uploads.\n("+extData.name+" : "+extData.size.byte()+")");
						}else if(errorType == "fileCount"){
							alert("Too many items are uploaded excess will not upload.");
						}
					}
				});
				// changeConfig
